            ...
            if job_id:
                existing = sorted(
                    [
                        ...
                        if f.startswith(f"{job_id}_chunk_")
                    ]
                )
                if existing:
                    job_logger.info(
                        f"Found {len(existing)} existing chunks. Skipping chunk generation."
                    )
                    chunks = existing
            ...
            if not chunks:
                if job_id:
                    log_stage(
                        ...
                        "chunking",
                        "running",
                        "Splitting video into chunks (this may take a moment)...",
                        chunk_id=None,
                    )
                chunks = self.split_video_with_overlap(
                    self.video_path, job_id, chunk_duration, overlap
                )
                if job_id:

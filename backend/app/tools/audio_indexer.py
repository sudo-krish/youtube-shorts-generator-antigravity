...
def get_audio_asset(semantic_query: str) -> str:
    ...
    if not os.path.exists(MANIFEST_PATH):
        logger.warning(
            "Manifest not found! Ensure download_seed_assets.py has been run."
        )
    ...
    if best_score > 0.4:
        logger.info(
            f"Local asset match found for '{semantic_query}': {best_match} (score: {best_score:.2f})"
        )
        return best_match
    ...
    if not FREESOUND_API_KEY:
        logger.warning(
            f"No strong local match for '{semantic_query}' and FREESOUND_API_KEY missing. Falling back to best available: {best_match}"
        )
        return best_match

    logger.info(
        f"No local match found for '{semantic_query}'. Attempting FreeSound API JIT fetch..."
    )
    ...
    try:
        ...
        if data.get("results"):
            ...
            manifest[relative_target_path] = {
                "tags": f"{semantic_query} downloaded dynamic {safe_filename.lower()}"
            }
    ...
    logger.warning(
        f"Could not source asset for '{semantic_query}'. Using fallback: {best_match}"
    )
    return best_match

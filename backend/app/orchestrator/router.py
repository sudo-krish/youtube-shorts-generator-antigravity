...
from app.orchestrator.manager import AIOrchestrator
import os
import logging
import uuid
import asyncio
...
def run_orchestrator_job(job_id: str, video_path: str, metadata: dict, resume_state: dict = None):
    ...
    try:
        ...
        db.jobs.update_status(job_id, "completed", json_path=output_path)

    except Exception as e:
        import traceback
        traceback.print_exc()
        job_logger.error(f"Error in background task: {e}")
        db.jobs.update_status(job_id, f"failed: {str(e)}")
        db.jobs.fail_running_stages(job_id)
...
async def start_analysis(request: AnalyzeRequest, background_tasks: BackgroundTasks):
    ...
    video_record = db.videos.get(request.video_id)
    if not video_record:
        raise HTTPException(status_code=404, detail="Video not found")

    job_id = str(uuid.uuid4())
    db.jobs.create(job_id, request.video_id, request.metadata)
...
async def redrive_job(job_id: str, background_tasks: BackgroundTasks):
    ...
    completed_stages = db.jobs.get_completed_stages(job_id)
    ...
    db.jobs.update_status(job_id, "processing")
    background_tasks.add_task(
        run_orchestrator_job,
        job_id=job_id,
...
async def cancel_job(job_id: str):
    ...
    db.jobs.update_status(job_id, "failed: cancelled by user")
    db.jobs.fail_running_stages(job_id)
    return {"status": "success", "message": "Job cancelled"}

@router.get("/jobs")
async def list_jobs_endpoint():
    return {"status": "success", "jobs": db.jobs.get_all()}

@router.get("/jobs/{job_id}/stages")
async def get_job_stages_endpoint(job_id: str):
    return {"status": "success", "stages": db.jobs.get_stages(job_id)}
...
async def get_status(job_id: str):
    ...
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    stages = db.jobs.get_stages(job_id)
...
async def render_batch(job_id: str, request: BatchRenderRequest):
    ...
    for variant_id in request.variants:
        ...
        db.jobs.queue_render(task_id, job_id, variant_id)
...
async def get_renders_endpoint(job_id: str):
    return {"renders": db.jobs.get_renders(job_id)}

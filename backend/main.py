...
from ai_director.orchestrator import AIOrchestrator
...
def run_orchestrator_job(
    job_id: str, video_path: str, metadata: dict, resume_state: dict = None
):
    ...
    try:
        ...
        # Stage 1: AI Director
        orchestrator = AIOrchestrator(video_path=video_path, metadata=metadata)
        output_json = orchestrator.orchestrate_pipeline(
            job_id, resume_state=resume_state
        )
...
async def get_models():
    try:
        ...
        all_models = sorted(list(set(gemini_models + deepseek_models)))
        return {"status": "success", "models": all_models}
    except Exception as e:
...
async def analyze_video(request: AnalyzeRequest, background_tasks: BackgroundTasks):
    logger.info(
        f"Starting async AI analysis on video {request.video_id} with metadata {request.metadata}..."
    )
...
async def get_node_output(job_id: str, node_id: str):
    ...
    if chunk_id:
        file_path = os.path.join(
            OUTPUTS_DIR, "agents", job_id, str(chunk_id), f"{stage_name}.txt"
        )
    else:
    ...
    while True:
        try:
            ...
            for idx, phase in enumerate(short.get("phases", [])):
                phase_id = phase.get("phase_id", f"phase_{idx}")
                clip_path = os.path.join(
                    OUTPUTS_DIR, vid, f"{vid}_{v_id}_{idx}_{phase_id}.mp4"
                )
                if os.path.exists(clip_path):
            ...
            if len(variant_clips) == len(short.get("phases", [])):
                ...
                out_file = os.path.join(
                    OUTPUTS_DIR, f"viral_short_{job_id}_{variant_id}.mp4"
                )
                try:
                    ...
                    if len(final_outputs) > 0:
                        update_render_status(
                            task_id, "completed", outputs=final_outputs
                        )
                    else:
                        logger.warning(
                            f"Tree Generator Failed for {task_id}. Triggering Redrive (Requeueing)..."
                        )
                        update_render_status(
                            task_id, "failed", "Tree Generation / QA Failed"
                        )
...
async def render_batch(job_id: str, request: BatchRenderRequest):
    ...
    for variant_id in request.variants:
        ...
        await render_queue.put(
            {"job_id": job_id, "variant_id": variant_id, "task_id": task_id}
        )
...
async def install_sfx():
    ...
    sfx_manifest = {
        "impact.mp3": [
            ...
            "lavfi",
            "-i",
            "aevalsrc='sin(400*exp(-t*4)*t)*exp(-t*2)':d=1",
            "-af",
            "volume=5",
        ],
        "riser.mp3": [
        ...
        "whoosh.mp3": [
            ...
            "lavfi",
            "-i",
            "anoisesrc=c=pink:d=1.5",
            "-af",
            "aeval='val*sin(t*3)*exp(-t*3)',volume=5",
        ],
    }

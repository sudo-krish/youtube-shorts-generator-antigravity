...
def validate_structure(video_path: str) -> bool:
    ...
    try:
        cmd = [
            ...
            "format=duration:stream=codec_type", "-of", "json", video_path
        ]
...
def validate_visuals(video_path: str) -> bool:
    ...
    try:
        ...
        if avg_brightness < 5.0: # Very dark threshold
            logger.error(f"QA Gate Failed: Video is completely dark (avg brightness: {avg_brightness:.2f})")
            return False
        ...
        if avg_diff < 0.5: # Extremely low variance
            logger.error(f"QA Gate Failed: Video appears frozen (avg frame diff: {avg_diff:.2f})")
            return False
            
        logger.info(f"QA Gate Passed Visual Check. Brightness: {avg_brightness:.2f}, Variance: {avg_diff:.2f}")
        return True

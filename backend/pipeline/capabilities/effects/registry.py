from .visual import (GlitchEffect, VignettePulseEffect, DesaturateEffect, 
                     BlackAndWhiteEffect, FlashBangEffect, VHSEffect, 
                     MotionBlurEffect, DynamicGlowEffect, DeepfriedEffect)
...
def get_capabilities_menu() -> str:
    ...
    for eff_name, cls in EFFECT_CLASSES.items():
        meta = cls.get_metadata() if hasattr(cls, 'get_metadata') else {"name": eff_name, "description": "No description.", "usage_scenario": "N/A"}
        menu += f"- **{meta['name']}**\n"

...
import subprocess
import ffmpeg
from modules.media.editor.engine import execute_pipeline, _build_visual_filtergraph
from modules.media.editor.qa_gate import run_qa_gate
...
def _stitch_variant(
    ...
    file_manager.write_text("tmp", Path(concat_txt).name, f"file '{file_manager.get_absolute_path('video_chunk', Path(hook_out).name)}'\nfile '{file_manager.get_absolute_path('video_chunk', Path(body_output_path).name)}'\n")
    ...
    try:
        file_manager.delete_file("tmp", Path(concat_txt).name)
    except OSError as e:
...
def generate_asset_tree(clips_data: dict, base_output_path: str) -> list[str]:
    ...
    for variant in hook_variations:
        ...
        try:
            file_manager.delete_file("tmp", Path(hook_out).name)
        except OSError as e:
            logger.warning(f"Failed to cleanup hook_out {hook_out}: {e}")

    # Cleanup Body Cache
    try:
        file_manager.delete_file("tmp", Path(body_output_path).name)
    except OSError as e:

...
def get_previous_iframe(video_path, target_time):
    # Stub for iframe snapping logic to keep cuts fast
    return target_time
...
def _prep_clip(
    ...
    out_file = file_manager.get_absolute_path("video_chunk", f"{base_name}.mp4")
    json_file = file_manager.get_absolute_path("video_chunk", f"{base_name}.json")
    ...
    for short in shorts:
        ...
        for idx, phase in enumerate(phases):
            clip_path = _prep_clip(
                phase, previous_phase, video_path, video_id, variant_id, idx, None
            )

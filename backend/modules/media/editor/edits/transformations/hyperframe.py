...
def generate_crop_polynomial(video_path, target_w=1080):
    ...
    if len(times) < 10:
        logger.warning(
            f"Not enough motion detected in {video_path}, using static center crop."
        )
        return f"(in_w-{target_w})/2"
...
def generate_polynomial_from_keyframes(keyframes: list, target_w=1080, orig_w=1920) -> str:
    """Generates an FFmpeg crop polynomial expression from a list of YOLO keyframes."""
    if not keyframes or len(keyframes) < 2:
        if keyframes:
            static_x = float(keyframes[0].get("x", orig_w / 2))
            cx_clamped = max(target_w // 2, min(orig_w - target_w // 2, static_x))
            return str(int(cx_clamped - (target_w // 2)))
        return f"(in_w-{target_w})/2"
    ...
    for kf in keyframes:
        ...
        cx = float(kf.get("x", orig_w / 2))
        cx_clamped = max(target_w // 2, min(orig_w - target_w // 2, cx))
        times.append(t)
        x_coords.append(cx_clamped - (target_w // 2))

    degree = min(2, len(times) - 1)
    z = np.polyfit(times, x_coords, degree)

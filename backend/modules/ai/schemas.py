...
class FocusKeyframe(BaseModel):
    t: float = Field(description="Relative time in float seconds from the start of the phase.")
    x: float = Field(description="The spatial X coordinate (0-1920) of the player focus point.")
...
class VideoPhase(BaseModel):
    ...
    focus_keyframes: List[FocusKeyframe] = Field(
        default=[],
        description="Dynamic second-by-second panning keyframes relative to the start of the phase.",
    )

...
sequenceDiagram
    ...
    participant API as FastAPI Backend (api/router.py)
    participant Worker as Async Task Queue
    participant AI as DeepSeek + Transformers
    participant FFmpeg as FFmpeg Render Engine
    participant DB as core/db/manager.py (SQLite)

    UI->>API: POST /api/upload (Raw VOD .mp4)
    API->>UI: Return video_id
    UI->>API: POST /api/analyze (Metadata: Game, Region, Vibe)
    API->>Worker: Trigger Pipeline (AIOrchestrator)
    Worker->>Worker: Pre-Process: Track, Audio Hype, MatrixBuilder
    Worker->>AI: 6-Stage Assembly Line (app/agents)
    AI-->>DB: Cache individual agent states
    AI-->>Worker: Return FactoryTimeline JSON
    Worker-->>UI: WebSocket Progress Streams
    UI->>API: POST /api/generator/render_tree
...
FastAPI routes traffic through a Django-style modular architecture.
- **`backend/api/router.py`**: The central "urls.py" connecting all domains.
- **`backend/app/`**: Domain specific logic (e.g. `admin`, `orchestrator`, `generator`, `upload`).
- **`backend/core/queue.py`**: A global asynchronous queue managing the execution of FFmpeg workers.

## 3. The Database Abstraction Layer (`core/db/manager.py`)
All database operations flow through a monolithic ORM-like structure instantiated as `db`. The database operates strictly in `WAL` mode with a centralized connection pool to prevent `database is locked` errors during intense multiprocessing. Raw SQL is forbidden outside of this manager.

## 4. The AI Layer (`app/agents/`)
...
## 5. Asset Engine Layer (Bootstrapping & JIT)
The pipeline employs a **Two-Tier Asset Strategy** to bypass heavy vector database dependencies:
1. **Local Cache & BM25 Matcher**: A local `workspace/sfx` directory seeded with high-quality CC0 `wav` files. The system utilizes a lightweight Python token-overlap search.
2. **JIT API Fetcher**: If the semantic match score drops below 0.4, the system triggers a Just-In-Time REST query to the FreeSound API, strictly filtering by CC0, and seamlessly streams the asset down.

## 6. Execution Engine Layer (`app/generator/`)

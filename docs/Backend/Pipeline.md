...
- **Language Fallback Mechanism**: If WhisperX hallucinates a rare language code (e.g. `haw` for Hawaiian) due to silence/noise and fails to locate an alignment model, a `try-except` block safely intercepts the ValueError and defaults to loading the `en` (English) alignment model.

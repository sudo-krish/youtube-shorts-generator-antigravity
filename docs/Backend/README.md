...
- **Relational Database**: State tracking is managed by a centralized SQLite database (`database.py`):
  - `videos`: Tracks physical files on disk via a `video_id` UUID.
  - `jobs`: Tracks execution Sessions, linking a `job_id` to a `video_id`.
  - `job_stages`: Tracks the hyper-granular states of the AI Assembly line (start times, end times, chunk logs, statuses).
- **Execution Logs**: Every job automatically writes isolated execution logs to `backend/outputs/logs/{session_id}.log`.
- **Redrive Capability**: When an agent fails, you can redrive from the exact failure point via `POST /api/redrive/{job_id}`.
